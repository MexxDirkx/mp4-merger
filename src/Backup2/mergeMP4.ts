...
export class SafeMSE {
  ...
  private pump() {
    ...
    const isInitAppend = this.hasInit && !this.initAppended;
    ...
    const onOk = () => {
      ...
      if (!isInitAppend && delta > 0.005) {
        ...
        console.log('Pushed boundary:', newEnd, 'boundaries now:', this.boundaries, 'labels:', this.labels);
      } else if (isInitAppend) {
        // Record boundary at 0 for init
        this.boundaries.push(0);
        console.log('Pushed init boundary at 0');
      }
  ...
  getLabelForTime(t: number): string {
    ...
    console.log('getLabelForTime:', t, 'boundaries:', this.boundaries, 'labels:', this.labels);
